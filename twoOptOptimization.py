...
def twoOptRoute(route, dst_mat):
    ...
    while decreasing:
        ...
        for i, j in itertools.combinations(range(1, len(best_route)), 2):
            new_route = twoOptSwitch(best_route, i, j)
...
def twoOptVRPSolution(file_path, solution):
    ...
    for route in solution:
        ...
        if route[-1] != '0':
            route.append('0')
        best_route = twoOptRoute(route, dst_mat)
        #remove depot
...
if __name__ == "__main__":
    ...
    new_solution = twoOptVRPSolution(file_path=file_path, solution=solution)
